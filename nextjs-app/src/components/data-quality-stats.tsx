...
import type { DetailedStats } from '@/lib/xlsx-processor';
    ...
    <Card>
      ...
      <CardHeader className="pb-4 border-b">
        <CardTitle className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold">Kvalita dat a statistiky</h3>
            <p className="text-sm text-muted-foreground font-normal">Přehled zpracovaných dat a doporučení</p>
          </div>
        </CardTitle>
      </CardHeader>
      ...
      <CardContent className="space-y-6 pt-6">
        {/* 1. BASIC METRICS */}
        <div>
          <h4 className="text-sm font-semibold text-foreground uppercase tracking-wide mb-3">Základní metriky</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-muted rounded-xl p-4 text-center">
              <p className="text-2xl font-bold text-primary">{totalProducts}</p>
              <p className="text-xs text-muted-foreground mt-1">Zpracovaných produktů</p>
            </div>
            <div className="bg-muted rounded-xl p-4 text-center">
              <p className="text-2xl font-bold text-primary">{avgRelatedProducts}</p>
              <p className="text-xs text-muted-foreground mt-1">Ø souvisejících</p>
            </div>
            <div className="bg-muted rounded-xl p-4 text-center">
              <p className="text-2xl font-bold text-primary">{avgAlternativeProducts}</p>
              <p className="text-xs text-muted-foreground mt-1">Ø podobných</p>
            </div>
            <div className="bg-muted rounded-xl p-4 text-center">
              <p className="text-2xl font-bold text-primary">{fullRelatedPercent}%</p>
              <p className="text-xs text-muted-foreground mt-1">s plným počtem (10)</p>
            </div>
          </div>
        ...
        {categoryAnalysis.length > 0 && (
          <div>
            <h4 className="text-sm font-semibold text-foreground uppercase tracking-wide mb-3">Analýza kategorií (TOP 5)</h4>
            <div className="overflow-hidden rounded-xl border">
              <table className="w-full text-sm">
                <thead className="bg-muted">
                  <tr>
                    <th className="text-left py-2 px-3 font-medium text-muted-foreground">Kategorie</th>
                    <th className="text-center py-2 px-3 font-medium text-muted-foreground">Produktů</th>
                    <th className="text-center py-2 px-3 font-medium text-muted-foreground">Ø souvisejících</th>
                    <th className="text-center py-2 px-3 font-medium text-muted-foreground w-12">Stav</th>
                  </tr>
                </thead>
                <tbody>
        ...
        {hasProblems && (
          <div>
            <h4 className="text-sm font-semibold text-foreground uppercase tracking-wide mb-3">Problémové oblasti</h4>
                ...
                <div className="bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-amber-800 dark:text-amber-400">Kategorie s méně než 10 produkty</p>
                      <p className="text-xs text-amber-600 dark:text-amber-500 mt-1">Doporučujeme rozšířit pro lepší propojení</p>
              ...
              {productsWithoutCategoryText > 0 && (
                <div className="bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
                  <div className="flex items-start gap-3">
                    <AlertTriangle className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="font-medium text-amber-800 dark:text-amber-400">{productsWithoutCategoryText} produktů bez categoryText</p>
                      <p className="text-xs text-amber-600 dark:text-amber-500 mt-1">Tyto produkty nemají přiřazeny podobné produkty</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {singleProductSubcategories.length > 0 && (
                <div className="bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
                  <div className="flex items-start gap-3">
                    <AlertTriangle className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
                    <div className="flex-1">
                      <p className="font-medium text-amber-800 dark:text-amber-400">Podkategorie s pouze 1 produktem</p>
                      <p className="text-xs text-amber-600 dark:text-amber-500 mt-1">Nelze přiřadit podobné produkty ve stejné podkategorii</p>
                      <div className="mt-2 flex flex-wrap gap-2">
